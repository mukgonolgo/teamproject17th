...
document.addEventListener("DOMContentLoaded", function() {
	const premiumAdSection = document.getElementById('premium-ad');
	const cards = premiumAdSection.getElementsByClassName('card');

	if (cards.length === 0) {
		premiumAdSection.style.display = 'none';
	}
});
...
function filterStores() {
	...
	if (selectedRegion === '') {
		...
		window.location.href = '/store/list'; // 전체 리뷰로 이동
	} else {
		window.location.href = '/store/list?region=' + encodeURIComponent(selectedRegion); // 하위 지역을 포함하여 리뷰 페이지로 이동
	}
}
